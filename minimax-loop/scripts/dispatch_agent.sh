#!/usr/bin/env bash
#================================================================
# MiniMax Loop Dispatcher (Standalone - No Shared Templates)
#================================================================

SKILL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MODE="${1:-}"
LOOP_NAME="${2:-DEFAULT_MINIMAX_ROADMAP}"
WORKSPACE="${WORKSPACE:-$(pwd)}"

export WORKSPACE LOOP_NAME MODE

STATE_DIR="${WORKSPACE}/.minimax-loop/state/${LOOP_NAME}"
LOG_DIR="${STATE_DIR}/dispatch_logs"
mkdir -p "${STATE_DIR}" "${LOG_DIR}"

dispatch_log="${LOG_DIR}/minimax-dispatch_${LOOP_NAME}_$(date +%Y%m%d_%H%M%S).log"

log_dispatch() {
    local timestamp
    timestamp=$(date '+%Y-%m-%d %H:%M:%S')
    echo "[${timestamp}] [PROVIDER:minimax] $1" | tee -a "${dispatch_log}"
}
...
build_prompt() {
    ...
    if [[ "$mode" == "optimize" ]]; then
        cat <<EOF
You are the AgentHUD ${role} agent for MiniMax.
Workspace: ${WORKSPACE}

BEFORE Optimize:
1. Read .minimax-loop/state/${LOOP_NAME}/failure_bank.json to avoid past errors.
2. Apply local_patches from .minimax-loop/state/${LOOP_NAME}/active_task.json.

Task: Follow .minimax-loop/state/${LOOP_NAME}/${LOOP_NAME}.md and update .minimax-loop/state/${LOOP_NAME}/sub-tasks/*.json.

CRITICAL INSTRUCTIONS:
1. Complete precisely one verifiable slice of work.
2. MANDATORY: git add and git commit your changes before finishing.
3. Update .minimax-loop/state/${LOOP_NAME}/${LOOP_NAME}.md with the latest status.
EOF
    else
        cat <<EOF
You are the AgentHUD ${role} agent for MiniMax.
...
Task: Inspect recent changes and update .minimax-loop/state/${LOOP_NAME}/${LOOP_NAME}.md.
...
invoke_agent() {
    ...
    mmx text chat --non-interactive --quiet --output text --message "$(cat "${prompt_file}")" 2>&1 | tee -a "${dispatch_log}"
    return ${PIPESTATUS[0]}
}
...
main() {
    ...
    local loop="${2:-DEFAULT_MINIMAX_ROADMAP}"

    log_dispatch "=== MiniMax Dispatch Start ==="
