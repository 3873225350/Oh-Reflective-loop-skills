...
INTERVAL="${MINIMAX_LOOP_INTERVAL:-60}"
...
if [[ -f "${PID_FILE}" ]]; then
    ...
    if [[ -n "${EXISTING_PID}" ]] && kill -0 "${EXISTING_PID}" 2>/dev/null; then
        echo "minimax-loop daemon is already running with PID ${EXISTING_PID}"
        exit 0
    fi
...
echo "Starting minimax-loop daemon for ${LOOP_NAME} (interval=${INTERVAL}s)"

# Determine launcher (tmux preferred, nohup fallback)
LAUNCHER="${MINIMAX_LOOP_LAUNCHER:-auto}"
...
SESSION_NAME="minimax-loop-${LOOP_NAME}-$(printf '%s' "${WORKSPACE}" | md5sum | cut -c1-8)"

if [[ "${LAUNCHER}" == "tmux" ]]; then
    if tmux has-session -t "${SESSION_NAME}" 2>/dev/null; then
        echo "minimax-loop tmux session already exists: ${SESSION_NAME}"
        exit 0
    fi
    ...
    echo "Started minimax-loop in tmux session: ${SESSION_NAME}"
else
    nohup env \
        WORKSPACE="${WORKSPACE}" \
        LOOP_NAME="${LOOP_NAME}" \
    ...
    echo "Started minimax-loop daemon (PID ${NEW_PID})"
fi
