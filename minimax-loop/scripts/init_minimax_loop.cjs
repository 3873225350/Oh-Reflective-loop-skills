...
async function run() {
  ...
  console.log(`[MiniMax-Loop] Initializing Reflective Loop environment in: ${workspace} for loop: ${loopName}`);

  const stateDir = path.join(workspace, '.minimax-loop', 'state', loopName);
  ...
  if (!fs.existsSync(roadmapPath)) {
    fs.writeFileSync(roadmapPath, `# MiniMax-Loop Integration Roadmap (${loopName})
...
`);
  ...
  const promptPath = path.join(workspace, '.minimax-loop', 'prompt.md');
  if (!fs.existsSync(promptPath)) {
    fs.writeFileSync(promptPath, `You are the MiniMax Loop Scheduler (Reflective Edition).
...
`);
  ...
  console.log(`[MiniMax-Loop] Done. You can now start the daemon: bash .minimax-loop/scripts/run_daemon.sh ${loopName}`);
}
