...
async function run() {
  ...
  console.log(`[Cursor-Loop] Initializing Reflective Loop environment in: ${workspace} for loop: ${loopName}`);

  const stateDir = path.join(workspace, '.cursor-loop', 'state', loopName);
  ...
  if (!fs.existsSync(roadmapPath)) {
    fs.writeFileSync(roadmapPath, `# Cursor-Loop Integration Roadmap (${loopName})
...
`);
  ...
  const promptPath = path.join(workspace, '.cursor-loop', 'prompt.md');
  if (!fs.existsSync(promptPath)) {
    fs.writeFileSync(promptPath, `You are the Cursor Loop Scheduler (Reflective Edition).
...
`);
  ...
  console.log(`[Cursor-Loop] Done. You can now start the daemon: bash .cursor-loop/scripts/run_daemon.sh ${loopName}`);
}
