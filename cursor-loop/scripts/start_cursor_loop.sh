...
INTERVAL="${CURSOR_LOOP_INTERVAL:-60}"
...
if [[ -f "${PID_FILE}" ]]; then
    ...
    if [[ -n "${EXISTING_PID}" ]] && kill -0 "${EXISTING_PID}" 2>/dev/null; then
        echo "cursor-loop daemon is already running with PID ${EXISTING_PID}"
        exit 0
    fi
...
echo "Starting cursor-loop daemon for ${LOOP_NAME} (interval=${INTERVAL}s)"

# Determine launcher (tmux preferred, nohup fallback)
LAUNCHER="${CURSOR_LOOP_LAUNCHER:-auto}"
...
SESSION_NAME="cursor-loop-${LOOP_NAME}-$(printf '%s' "${WORKSPACE}" | md5sum | cut -c1-8)"

if [[ "${LAUNCHER}" == "tmux" ]]; then
    if tmux has-session -t "${SESSION_NAME}" 2>/dev/null; then
        echo "cursor-loop tmux session already exists: ${SESSION_NAME}"
        exit 0
    fi
    ...
    echo "Started cursor-loop in tmux session: ${SESSION_NAME}"
else
    nohup env \
        WORKSPACE="${WORKSPACE}" \
        LOOP_NAME="${LOOP_NAME}" \
    ...
    echo "Started cursor-loop daemon (PID ${NEW_PID})"
fi
