---
name: cursor-loop
description: Sets up and manages an autonomous scheduler loop based on reflective machine learning metaphors (Pretrained Backbone, Local Adapters, Error Banks). Use when asked to setup or manage a cursor-loop.
---

# Cursor Loop Skill (Reflective ML Edition)

This skill provides an automated framework for running an autonomous coding loop using Cursor, heavily inspired by ML training principles.
...
|--------|---------------|
| **CLI Flags** | Cursor CLI with `--agent` mode and workspace context |
| **Model Fallback** | Cursor models (configurable) |
| **Session** | Cursor workspace session management |
...
| Script | Purpose |
|--------|---------|
| `scripts/dispatch_agent.sh` | Dispatch with Cursor-specific invoke_agent function |
| `scripts/run_daemon.sh` | Daemon loop with mode switching |
| `scripts/init_cursor_loop.cjs` | Initializes a new loop instance |
...
# Manual dispatch
bash cursor-loop/scripts/dispatch_agent.sh <optimize|check> <LOOP_NAME>
```