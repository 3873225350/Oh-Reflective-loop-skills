...
def setup_environment():
    ...
    instance_log = os.path.join(LOG_DIR, f"moa-loop-{plan_id}-PID{pid}-{start_time}.log")
...
def log(msg, plan_id, pid, instance_log, indent=0, sub_pid="-----"):
    ...
    formatted = f"[{timestamp}] {header} [PROVIDER:moa] {prefix}{msg}\n"
    print(formatted, end="")
...
def detect_available_agents():
    ...
    for agent, cmd in commands.items():
        try:
            result = subprocess.run(["which", cmd], capture_output=True, text=True)
            agents[agent] = result.returncode == 0
        except:
            agents[agent] = False

    return [a for a, v in agents.items() if v]
...
def run():
    ...
    primary_agent = available_agents[0] if available_agents else "gemini"

    log("╔════════════════════════════════════════════════════════════════", plan_id, pid, instance_log)
    log("║ SESSION START: MOA Loop Orchestrator (Mixture of Agents)", plan_id, pid, instance_log)
    log(f"║ Plan ID: {plan_id}", plan_id, pid, instance_log)
    log(f"║ Available Agents: {', '.join(available_agents) if available_agents else 'None'}", plan_id, pid, instance_log)
    log(f"║ Primary Agent: {primary_agent}", plan_id, pid, instance_log)
    ...
    while True:
        ...
        try:
            ...
            log(f"MOA Dispatcher Active", plan_id, pid, instance_log, indent=1, sub_pid=process.pid)
