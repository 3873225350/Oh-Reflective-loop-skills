...
async function run() {
  ...
  console.log(`[Kimi-Loop] Initializing Reflective Loop environment in: ${workspace} for loop: ${loopName}`);

  const stateDir = path.join(workspace, '.kimi-loop', 'state', loopName);
  ...
  if (!fs.existsSync(roadmapPath)) {
    fs.writeFileSync(roadmapPath, `# Kimi-Loop Integration Roadmap (${loopName})
...
`);
  ...
  const promptPath = path.join(workspace, '.kimi-loop', 'prompt.md');
  if (!fs.existsSync(promptPath)) {
    fs.writeFileSync(promptPath, `You are the Kimi Loop Scheduler (Reflective Edition).
...
`);
  ...
  console.log(`[Kimi-Loop] Done. You can now start the daemon: bash .kimi-loop/scripts/run_daemon.sh ${loopName}`);
}
