...
async function run() {
  ...
  console.log(`[Qwen-Loop] Initializing Reflective Loop environment in: ${workspace} for loop: ${loopName}`);

  const stateDir = path.join(workspace, '.qwen-loop', 'state', loopName);
  ...
  if (!fs.existsSync(roadmapPath)) {
    fs.writeFileSync(roadmapPath, `# Qwen-Loop Integration Roadmap (${loopName})
...
`);
  ...
  const promptPath = path.join(workspace, '.qwen-loop', 'prompt.md');
  if (!fs.existsSync(promptPath)) {
    fs.writeFileSync(promptPath, `You are the Qwen Loop Scheduler (Reflective Edition).
...
`);
  ...
  console.log(`[Qwen-Loop] Done. You can now start the daemon: bash .qwen-loop/scripts/run_daemon.sh ${loopName}`);
}
