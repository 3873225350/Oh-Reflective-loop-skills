...
3. Update .codex-loop/state/${LOOP_NAME}/${LOOP_NAME}.md with the latest status.
EOF
    else
        cat <<EOF
You are the AgentHUD ${role} agent for Codex.
...
invoke_agent() {
    ...
    if [[ -f "$THREAD_FILE" ]] && [[ -s "$THREAD_FILE" ]]; then
        ...
        codex --resume "$(cat "$THREAD_FILE")" "$(cat "${prompt_file}")" 2>&1 | tee -a "${dispatch_log}"
    else
        log_dispatch "Starting new thread for mode: ${mode}"
        codex "$(cat "${prompt_file}")" 2>&1 | tee -a "${dispatch_log}"
    fi
    return ${PIPESTATUS[0]}
}
